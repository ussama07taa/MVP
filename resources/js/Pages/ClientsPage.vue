          ...
          <div v-if="activeDossierTab === 'devis'">
            ...
            <div v-else class="space-y-3">
              <div v-for="inv in selectedClientDossier.invoices" :key="inv.id" class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                ...
                <div class="flex items-center gap-6">
                  <div class="text-right">
                    <p class="text-[9px] font-bold text-slate-400 uppercase">Payé / Total</p>
                    <p class="font-black text-slate-800 text-xs">
                      <span class="text-emerald-600">{{ Number(inv.amount_paid).toFixed(2) }}</span> / {{ Number(inv.total).toFixed(2) }} DH
                    </p>
                    <p v-if="inv.type === 'invoice' && (Number(inv.total) - Number(inv.amount_paid)) > 0.01" class="text-[10px] font-black text-red-500">
                      Reste: {{ (Number(inv.total) - Number(inv.amount_paid)).toFixed(2) }} DH
                    </p>
                  </div>
                  <div class="flex flex-col items-end gap-1.5">
                    ...
                    <button v-if="inv.type === 'invoice' && (Number(inv.total) - Number(inv.amount_paid)) > 0.01" 
                            ...
                            class="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg font-bold text-[10px] hover:bg-emerald-100 transition-all flex items-center">
                      <CreditCardIcon class="w-3 h-3 mr-1" /> Encaisser
                    </button>
                  </div>
                </div>
              </div>
...
const submitInvoicePayment = async () => {
  ...
  try {
    await axios.post(`/api/orders/${invoiceToPay.value.id}/pay`, { 
